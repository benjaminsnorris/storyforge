...
| `scene_index` | The master list of every scene with metadata (POV, location, status, word count, and any scene extensions). Stored as two pipe-delimited CSV files: `reference/scene-metadata.csv` (structural data) and `reference/scene-intent.csv` (creative intent). Legacy projects may still reference `scenes/scene-index.yaml`. | `reference/scene-metadata.csv` |
