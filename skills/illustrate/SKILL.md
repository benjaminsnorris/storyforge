...
**The upload list leads with the anchor batch.** The four approved images are ranked ahead of plan order and labeled with their slot (`anchor batch: darkest`), so the four renders phase 1 exists to sign off are the four the churn actually references. The cap is four *prior illustrations* and the cover is additive, so a full batch fits. Two things to say out loud if they appear: a batch member excluded as **pre-canon** is still excluded — being approved does not make a render that predates the current canon safe to reference, so re-render it — and a batch image dropped by the cap gets its own note naming its slot, which means the reference list is not what the author approved.
