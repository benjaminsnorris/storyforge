...
**It is a render, never hand-edited** — same as the packet. `cleanup` reports `illus_export_stale` when the export is older than the plan, the transition log, a canon file, or a prompt file it aggregated, **and** when a copied reference image no longer matches the source it was copied from. That last one is the reason the digests are in `manifest.json`, and it is the only way this bundle goes stale without anything under `reference/` moving: someone re-rendered an illustration the export was carrying as a reference.
