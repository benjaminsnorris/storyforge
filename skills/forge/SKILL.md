...
**1.1. Missing summaries at the current tier:** If the active level's CSV exists but has rows with empty `summary` cells (run `./storyforge score --level N` to see; the floor check "summary non-empty for every {event|anchor|scene}" will fail) → "Some {tier} rows are missing their one-sentence summary." → at full coaching, offer to run `./storyforge propose-summaries --level N` to draft candidates; at coach, suggest running it with `--coaching coach` to get a review brief; at strict, suggest running it with `--coaching strict` to get the constraint checklist. Recommend reading `reference/outline.md` after to see the expanding outline read top-to-bottom.
