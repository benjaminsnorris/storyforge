...
| `visual_state.py` | The visual-state transition log: read/write, forward resolution (`state_at`), the four-check deterministic pre-pass (`prepass`), digest drift (`digest_drift`), and audit provenance |
| `packet.py` | What goes in the handoff packet and proof the copies are exact: `resolve` (entries, anchors, references, and the `gaps` coverage record), `rows_in_reading_order`, `needs_render` (id → why it still needs a render, over one staleness predicate shared with `--prompts`' reference chain), `state_context` → `RowContext` / `state_for_row` / `contrast_for_row` (shared with `--prompts`, so the two artifacts cannot describe one row differently within a run), `state_grid`, `anchor_batch`, `anchor_block`, `anchor_copy_drift` (covers the export's `canon.md` too), `packet_stale`. `entry_for` / `book_level_gaps` / `audit_gaps` / `stale_render_gaps` are public because `export.py` builds its units from them. No rendering, no LLM |
| `export.py` | The self-contained per-illustration export: `resolve` (units, references-as-copy-instructions, and the same `gaps` record), the body recovered from `prompts/{id}.md` or derived from the plan row, `manifest_for`, `copy_references`, `prune_units` (which takes `live_ids` and refuses an empty set), `export_stale` (both ends of every digest, and every reason it could not reach a verdict). `ExportUnit` inherits `packet.Entry`. No rendering, no LLM |
| `prompts_export.py` | The export's two renderers — `render_prompt` (the contiguous paste block plus the per-image checks) and `render_readme` — and `render_zip_hint` |
| `prompts_packet.py` | The six packet renderers, the sequence pre-pass request + parser (`build_sequence_request`, `parse_sequence_response`), `duplicate_treatments`, and the coach brief / strict checklist for `--sequence` |
| `prompts_illustrate.py` | Illustration selection + art-direction prompt builders, `prompt_constraints` / `prompt_acceptance_lines` (shared with the export so the two paste-ready artifacts cannot state different constraints for one row) and `parse_prompt_file` (the reader of `render_prompt_file`, kept beside it), book-level canon-file builders (`CANON_PLAN` templates, filled-canon and anchor stubs, the direction request and its response parser), sequence-review renderer, coach brief, strict checklist, the visual-state request / brief / checklist, and the contradiction-audit request + report renderer |
...
**Entry shape: 80–120 words of *derived* content**, against `--prompts`' 250–400, because the shared sections carry the rest. The budget governs what the renderer emits — beat, subject, resolved state, the one-sentence contrast, the composition note — not the author's own `absent` and `contrast` cells, which sit on top of it and are theirs to spend (SKILL.md invites them, and a test documents that they may push an entry past 120). Only what is specific to this image: `Beat`, `In frame`, `State`, `Absent`, `Treatment`, `Contrast`, and an image-specific composition note. `State` stays in the entry because it is the matrix *resolved* for one scene (`state_at` overlaid with `parse_state_override`, narrowed to the row's `canon_refs`), not a duplication of the matrix — and an override naming an entity the row does not list is still included, since it was written for that image. What moved to `acceptance.md`: the orientation directive, no-lettering, the colour prohibitions, the eight-point accept check, and the sequence-contrast rules — every check identical across the set, stated once. `Absent` and `contrast` read author-written columns the schema does not define (`write_plan` preserves extras), deliberately not added to `PLAN_COLUMNS` because nothing populates them and a write-only column is the `embeds_as` mistake — `skills/illustrate/SKILL.md`'s Package mode is where an author learns they exist. `_contrast_for` emits **one** derived sentence (register plus predecessor), not three: the spec's own `lamp-relit` example renders at 116 words with every field populated, and three stacked sentences here spent a tenth of the budget restating two facts. The budget test uses that example rather than the fixture's short cells, which rendered to 63 words and could not fail.
...
**The self-contained export** — `--export` assembles `manuscript/illustration-export/`: `README.md`, `canon.md`, `acceptance.md` at the root, plus one directory per illustration holding `prompt.md`, `references/`, and `manifest.json` (#298). Not a replacement for the packet and not a compromise of it — a second artifact for a second consumer, because the packet's economies are exactly what a single-image handover cannot afford.

**The packet deliberately does not aggregate the prompt bodies, and the spec is amended to say so.** `packet.py` never did — every entry is built from the plan row — and the design-spec sentence claiming the GN `script-package` precedent was wrong about this repo. Keeping it that way is the decision: the 80–120 word entry exists *because* hyper-detailed leaf prompts underperform, so inlining a 250–400 word body per entry would spend that budget twenty times over and rebuild the twenty-independent-prompts shape the packet replaced. What the packet does instead is name the `prompts/{id}.md` convention and `--export` **once**, in `illustrations.md`'s header — the path and the command are identical for every row, so per-entry copies would be the duplication the packet's whole economy is against.
...
| Reference images | project-relative paths, never copied | copied in, numbered in upload order, symlinks resolved |
| Staleness | `packet_stale` | `export_stale` — also prompt files, also reference-image digests |

**Both read the same derivation of a row.** `packet.entry_for`, `state_for_row`, and `contrast_for_row` are shared, which is #297's reasoning applied a second time: two renderings of one row is how they come to disagree about a costume, one function is how they cannot. `book_level_gaps` and `audit_gaps` take a `bundle` noun for the same reason a gap must not be wrong about where it is written — a sentence saying "this packet" inside `illustration-export/README.md` sends the reader to a directory that may not exist.

**The export re-derives the Constraints block; it inherits only the body.** `prompts_illustrate.parse_prompt_file` recovers the model-authored prose and stops at the trailer, and `prompt_constraints` regenerates the rest from the plan. That split is load-bearing: there is no `prompt_stale`, so a prompt file written before a matrix edit still carries the old state in its own Constraints block — the export exports the state in force *now*, and a block that contained both would contradict itself, which is #297 inside the fix. The parse is structural (the paste sentinel and the trailer headings, module constants shared with the writer) rather than marker-delimited, because HTML-comment markers around the body would sit *inside* the region the file tells the author to paste.

**The terminator matches any `##`–`####` level, and a repeated heading is reported rather than resolved.** A literal `'### Constraints'` missed a hand-promoted `## Constraints` — the likeliest hand edit, since every heading around it is `##` — and the file's own stale constraints then landed inside the exported paste block beside the fresh ones: two contradicting costume directives in one region a reader is told to paste whole, #297 reconstituted inside the split built to prevent it. A model that writes its own `### Constraints` is the mirror case, and neither reading of it is safe to make silently: cutting at the last match carries stale constraints in, cutting at the first drops whatever followed, usually `### Use case`. So the cut is at the **first** match, the half that cannot self-contradict, and a *repeated* trailer heading yields `body_truncated`, which `_body_for` turns into a gap. That is #293's rule — a truncation every consumer accepts is worse than an absence, because nothing looks wrong.

**The two paste-ready artifacts cannot drift in how they phrase a constraint; they can still differ in content.** `prompt_constraints` and `prompt_acceptance_lines` are shared, which is the phrasing guarantee. Two documented content differences remain by design: the export opts into `contrast` (the prompt file keeps it below its do-NOT-paste line, because the derived clause names a neighbouring illustration by id), and a prompt file predating a matrix edit keeps its old state bullet. The shared bullet says "consistent with its continuity anchor" rather than "the anchor description above", because in the export the anchors live in a sibling `canon.md` and a positional reference pointed at nothing.

**Reference copies are the point, and the recorded sha256 is what makes them safe.** The packet's objection to copying — a second thing to invalidate — is answered rather than dismissed: `manifest.json` records each copy's digest and `export_stale` re-hashes **both ends**, the source (a re-render replaced it) and the copy in the unit directory (the bundle is missing it or holding something else). Checking only the source left the file the reader actually uploads unverified, which is what made "a manifest never promises an upload the directory does not hold" an overclaim with no enforcement. Staleness with nothing under `reference/` touched is why this is its own finding kind rather than a second `packet_stale` whose detail would name the wrong command. Symlinks are resolved on copy and the resolved target recorded, because a zip preserving a dangling link is worse than a copy; `copyfile` rather than `copy2` because the copy is identified by its recorded digest and by nothing else, so carrying the source's mode bits and mtime into a bundle meant to be zipped imports metadata no consumer reads.

**Every reason `export_stale` cannot reach a verdict is a reason it reports.** An unreadable manifest, a `references` value that is not a list, an entry with no digest, an image that exists and cannot be read — all of them, because `--diagnose` renders silence here as `built and current`, which is `staleness_unchecked_finding`'s rule and `--audit`'s "Not assessed" applied to a third artifact. The unguarded version of that hash was worse than a silence: `ill.sha256_of` raised `PermissionError` out of `validate_plan`, and since that is the single finding collector, one unreadable image destroyed the whole illustration health report including the blocking findings `cmd_validate` gates on. Source digests are memoized per call — the cover is a reference in every unit, so twenty illustrations hashed it twenty times per `validate`.

**The shared files are written after the units.** `is_built` keys on those three, so writing them first flipped it True at the moment the bundle was emptiest: an interrupted run left `--diagnose` printing "built and current" over zero unit directories, with the count as the only tell. `is_built` is also deliberately *weaker* than `packet.is_built`, which requires the payload — a bundle with no units is legitimate here (an empty plan, a mid-flight `--ids` run), so the window is closed by the write order and by `export_stale` reporting a unit missing its `prompt.md`, not by widening the predicate.

**A unit with no prompt file is exported, and says so three times.** In `prompt.md`'s "Read this first", in `README.md`'s table (`plan row only`) and gap list, and in the log. Not refused — an export costs nothing to produce, so building it and stating what is thin is `run_package`'s posture — and not silent, because a block assembled from three plan cells reads exactly like a complete prompt. The stand-in keeps the 5-section shape so a reader sees at a glance which parts are unwritten.

**`--anchor-batch` makes phase 1 one command**, and reports how each slot was chosen. That reporting is not optional dressing: nothing populates `register`, so darkest and brightest are usually the first and last illustration in reading order, and the author who typed the flag never named the four ids — a guessed slot they cannot see is a decision they are unaware of accepting. `--anchor-batch` with `--ids` is refused rather than merged, and `--anchor-batch` without `--export` is refused rather than ignored, because silently dropping a scope is how a run meant to touch four rows touches twenty.

**A whole-plan export prunes; a subset export never does — and the flag that says which is not the flag that authorizes deletion.** `ExportScope` (`whole-plan` | `subset`) drives README's sentence; `live_ids`, the plan's own id set, is what `prune_units` may keep, and it refuses an empty one. Those were one `complete` bool, and the two questions diverge on exactly one input: `ill.read_plan` returns `[]` for a plan file that is missing, renamed, or mid-merge, so `set() >= set()` was vacuously True, and the whole bundle was deleted at exit 0 with a log line blaming rows that had not moved. README must still not call an empty plan a partial export — that is why the answer was two fields rather than a stricter bool. Only directories whose name is a legal illustration id *and* which hold a `manifest.json` this command wrote are candidates, so an author's own notes survive.

**An illegal plan `id` is refused before anything is written.** A unit is a directory named from that cell, and `copy_references` `rmtree`s inside it — so `../../evil` escaped the export tree and destroyed a directory nobody named. The plan is a documented hand-edit surface, so a typo reaches it, and `run_export` does not call `validate_plan` (its other findings would duplicate the gaps). `run_export` checks `ill._ID_RE` up front and `unit_dir` raises, so the one check whose absence is destructive cannot be bypassed by a later caller.

**One root cause, one gap; the per-unit sentence is a different channel.** A project-wide cause — no prompt files yet, a cover-only reference chain — is reported `N of M (ids)` once in `README.md` and once in the log, while the sentence for one image stays in that image's `prompt.md`. Emitted per row it was one aggregate plus twenty near-identical bullets in the section whose value is proportional to its signal-to-noise (#290). The reference chain's own disclosures reach the log and README too, and sit under `## About these reference images` rather than `## Read this first`: that section is for the four things that change whether generating now is a good idea, and an unavoidable note about a book with no ingested art yet dilutes them. `_references_for` takes a `rerun` flag, because its stale-render note told the export's reader to regenerate the packet.

**`acceptance.md` speaks each bundle's own vocabulary.** The eight checks are identical and written once, but they *point* at where the reader will find each field — and the packet's entries do not exist in the export, whose whole purpose is being readable without the repo. `prompts_packet.AcceptanceSource` selects the pointers; reused verbatim, the export told its reader to check the image against "the entry's **Absent** line". Same class as the `bundle` noun, applied to the renderer that was missed.

**`anchor_copy_drift` covers both trees.** Both write anchor copies through the same `anchor_block`, so both are under the same byte-identity invariant, and checking only the packet would leave the artifact designed to be *handed to someone else* unguarded. The remediation names whichever command regenerates the file, since `--package` does not rebuild the export.

**Reference images are never copied** into the packet; `reference-images.md` carries project-relative paths, because a copy is a second thing to invalidate. The list comes from `--prompts`' own `_references_for`, so the packet inherits the canon-gated exclusions rather than growing a second chain — and `_references_for` takes an optional `notes` out-list so those exclusions are *rendered* under "What is not in that list", not merely logged. That matters more than it looks: with every prior render excluded as pre-canon the list shrinks to the cover, which reads identically to a book where nothing has been rendered yet, and the author then uploads the cover alone and generates the rest with no likeness reference. Cover-only-with-ingested-art is also a README gap. The four-image cap (`_MAX_REFERENCES`) is disclosed too, and is checked *after* the exclusion checks so a stale render past the fourth reference is not hidden behind a cap that is not why it was dropped.

**A rendered entry is marked as one.** `Entry` carries `status`, and `render_entry` marks anything past `planned`/`prompted` in both the heading (`— already rendered`) and the metadata line. The documented flow renders the anchor batch, ingests it, and regenerates, so the normal mid-flight packet mixes finished and pending rows while README says to work the file top to bottom — identical shapes would have an author re-rendering finished art. `Entry.status` is typed `PlanStatus`, and `_RENDERED_STATUSES` is enumerated *positively* so an out-of-vocabulary value falls toward pending: reading an entry that needed no reading costs a glance, while skipping one because a typo made it look finished loses an illustration from the book.
...
**Unrendered is valid in-flight state**, not a finding — same posture as GN page renders (#261). `cleanup` reports genuine incoherence under "Interior Illustrations" (`illus_orphan_marker`, `illus_missing_file`, `illus_orphan_file`, `illus_anchor_drift`, `illus_duplicate_marker`, plus row-level schema problems) and the visual-state kinds (`illus_state_unknown_scene` — the only error of the six — plus `illus_state_unmapped_scene`, `illus_evidence_not_found`, `illus_state_unspecified`, `illus_prose_changed`, `illus_audit_stale`) and the bundle kinds (`illus_packet_stale`, `illus_export_stale`, `illus_anchor_copy_drift` — all warnings, since both bundles are renders and every one of the three is one `--package` or `--export` away) and `illus_canon_anchor_truncated` (blocking: a canon Embeddable block cut short by a `##` heading inside it, so every prompt embeds a shorter anchor than the file appears to hold — #293); `validate` fails on the blocking ones and warns on drift.
