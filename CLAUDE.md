...
**`prompt_spoils_unread`** (item 4) covers art already directed, which matters because 17 of 20 rows on the book this was filed about needed re-rendering anyway and every one was directed by a prompt built the old way. Six-word shingles, and **"distinctive" is a set difference, not a frequency heuristic**: `(body ∩ unread) - everything before the split`. The subtrahend is *not* `split['read']`, which is capped at `READ_CHARS` — subtracting the window flagged a refrain the reader met early in a long scene and met again after the anchor, and this invariant is what the finding cites to justify being a finding rather than a hint. Six is a **correctness threshold, not a tuning knob**: 4 floods a real book with false warnings and 9 stops catching the three rows the issue was filed about, and both left the whole suite green until a boundary test pinned it. Genuinely silent only for a row with no `prompt_file`, and for `establishing` / `at_scene_end` — everything else emits `prompt_spoiler_unchecked`.
