...
- `parse_yaml_scalar(raw)` — the one parser for the text after a `key:`. Strips the inline comment (YAML rules: a `#` only opens one at the start or after whitespace, and never inside quotes) and resolves the quoting, including `\xNN` / `\uNNNN`. `common._strip_yaml_value`, `prompts._strip_yaml_value`, `assembly._strip_yaml_quotes` and the scalar half of `visualize._read_yaml_field` all delegate to it — there were **four** quote-stripping copies, none of which removed a comment, so an inline comment reached the epub as part of the value, an *empty* field carrying a template comment read as truthy, and the dashboard rendered a title with its `# Working title` note attached (#277). `tests/test_yaml_helpers.py` parametrizes over every entry point, which is what keeps them from diverging again. Two deliberate behaviours that look like bugs: malformed quoting degrades to the lenient strip rather than to `''`, so a typo is a visible wrong title instead of a silently missing one; and an *unknown* escape keeps its backslash rather than returning a plausible-looking value (a defined escape like `\t` still resolves, which is correct YAML).
- `yaml_single_quote(value)` — escape a value for emission into YAML we generate, by doubling apostrophes. `generate_epub_metadata` wrapped values in single quotes with no escaping, so one apostrophe (`Children's`) closed the string early and pandoc exited 64 (#277). Folds `[\r\n]+` only — **not** all whitespace: `generate_epub_metadata` validates a cover path with `os.path.isfile` and then emits it through here, so collapsing every `\s+` run verified a real file and wrote an altered path. `_yaml_double_quote` is the counterpart for the `"..."` values `update_artifact_entry` writes.
- `yaml_scalar_issue(raw)` — **reporting only**: says whether a scalar was probably not read the way its author meant (`unterminated_quote` / `trailing_after_quote` / `comment_truncated`), or `None`. It never changes what `parse_yaml_scalar` returns and must not — both behaviours it flags are deliberate and correct, and the gap it closes is that both are *silent* (#315). `cleanup`'s `_check_yaml_scalars` is the consumer, because the report is the durable artifact; `parse_yaml_scalar` runs on every field read of every command, so a WARNING there would fire repeatedly for one typo. `comment_truncated` keys on a `#` followed immediately by a non-space (`Book #2`, `#ff8800`) — a deliberately conservative **heuristic**, since the shipped template puts a real `# like this` comment on nearly every key and reporting those would bury the finding on every project; `Book # 2` is therefore a known false negative. Tests assert zero findings on both the template and the fixture.
- `detect_newline(text)` — the terminator to use when writing a line into `text`, keyed on the **first** line ending. The one such predicate: three writers of `storyforge.yaml` each sniffing for themselves is three chances to disagree, and two of them did (one on "CRLF anywhere", one on the first line) while a docstring claimed they shared a policy. A genuinely mixed file is *reported* by `_check_crlf`, not repaired.
- **Line endings in `storyforge.yaml` are preserved, never converted, and CRLF is reported instead** (#314). One policy across all **three** writers: `update_artifact_entry`, `cmd_cleanup.migrate_storyforge_yaml`, and `cmd_write._replace_in_file` (which advances `phase`). If you add a fourth, use `detect_newline` and `newline=''`. This is the *opposite* of the CSV policy, where `write_plan` / `elaborate._write_csv` / `history.append_cycle` force LF via an explicit `lineterminator` — deliberately, because `storyforge.yaml` is a file the author hand-edits, and this codebase's standing posture for author-owned content is to report rather than silently mutate (an ambiguous anchor is reported, not re-placed; a superseded export directory is reported, not deleted). `_check_crlf` covers `storyforge.yaml` so an author who wants LF is told. Before this, `cleanup` normalized it as an undeclared side effect of an unconditional write, which defeated `update_artifact_entry`'s preservation and was reached by following that function's own remedy text. **Consequence for anything editing this file:** open with `newline=''` on both ends, build inserted blocks with the file's own newline (mixed endings are worse than either policy), write only when the content actually changed, and make patterns `\r?\n`-tolerant — `^artifacts:\n` does not match `artifacts:\r\n`, and a text-mode read used to hide that by normalizing first.
- `update_artifact_entry(project_dir, artifact, exists=, updated=)` — set those keys on one `artifacts:` entry **in place**, returning whether the file changed. Use this rather than a regex over the whole file: the code it replaced was a `re.sub` under `re.DOTALL` ending in an unanchored `.*`, which matched to end of file, so `assemble` deleted `phase`, `parts`, and the entire `production` block and then committed the truncation (#276). Preserves blank lines, key order, inline comments and CRLF endings; reads the child indent off the file rather than assuming two spaces; inserts a missing key inside the block rather than skipping it. Two things it does *not* preserve byte for byte, stated because the guarantee is otherwise easy to over-read: a comment's leading whitespace normalizes to two spaces on a line it rewrites, and a file with no final newline gains one. Note this **preserves** CRLF where `write_plan` **forces** LF for CSVs — opposite policies, deliberately: this edits a file the author wrote, so `cleanup`'s `crlf_line_endings` check reports `storyforge.yaml` (#314) instead of anything converting it.
...
**`SceneSplit` carries a named `state`, and consumers switch on it.** Four values, because three of them were originally derived from which fields happened to be empty — at five call sites across three modules, keyed on **three different fields** (`error`, `unread`, `next_sentence`) whose equivalence nothing asserted. That is exactly what `prompts_packet._entry_state` was created to prevent, and the state it was right about is `establishing`, which was spelled `read == ''` — also what the snapping bug below produced.

| `state` | Means | Request | Acceptance line |
|---|---|---|---|
| `normal` | prose on both sides | read prose + the forbidden block | quotes the next sentence |
| `establishing` | a declared `scene_open` opener | the *following* prose, as the thing to depict | states that no check applies |
| `at_scene_end` | nothing follows *in that scene* | read prose, no forbidden block | states that no check applies |
| `unknown` | position unresolved | says so; the prose is the scene's opening | says the check could not be made |

**`establishing` is keyed on the declared placement, never on `offset == 0`.** `before_anchor` returns the containing paragraph's start, so an anchor in the *first* paragraph resolved to 0 and was classified an opener — losing the forbidden block, the acceptance check, and `prompt_spoils_unread` entirely, and flipping on whether the scene file happened to begin with a heading. `offset == 0` was the same kind of proxy as the `read == ''` spelling this Literal exists to replace. That configuration is still legitimate and still reachable — a `before_anchor` image at a scene's very start has read nothing — so `normal` with an empty `read` is a real state; the request says so instead of claiming the reader has read an empty block, and `--prompts` names those rows, since the author almost always meant `scene_open`. What must never happen is an empty `read` at a *non-zero* offset, which is #308's shape: a leading blank line produced exactly that (offset 2, whitespace `read`, `state='normal'`), so the body is `lstrip('\n')`-ed and the invariant is `read == '' ⟹ offset == 0`.

`establishing`'s carve-out matters because getting it wrong was a live bug on every book: `read` is empty by construction, so the request asserted "this is the prose the reader has read" over an empty section, put the scene's opening in the **forbidden** block — telling the model to avoid the only description of the image it was being asked to draw — and `spoiler_findings` flagged every correct opener, because an empty read side makes the set difference vacuous. The remedy told the author to re-render correct art. `scene_open` + `full_page` is the documented full-page opener, so this was not an edge case.

`establishing` and `at_scene_end` emit **a stated absence** rather than a check — no box to tick, because rendering a vacuous check told the author to re-render a correct opener, but not silence either: `acceptance.md`'s page-turn item sends the author to the prompt file for a sentence, so emitting nothing made that a dead-end referral on exactly the states where no sentence exists. That is `NOT_RECORDED`'s doctrine, not an exception to it. (`at_scene_end` means nothing follows *in that scene*; the next scene's prose is unread and nothing checks it — a stated gap.) `unknown` is **stated**, in the request and in `## Accept only if`: silence would read as "there is nothing after this image", the same failure arrived at from the other direction. Same doctrine as `--audit`'s "Not assessed", `packet.NOT_RECORDED`, and `staleness_unchecked_finding`.

**The read window snaps to a paragraph boundary, and the bound is load-bearing.** Without `start < offset`, a paragraph longer than `READ_CHARS` means no paragraph *begins* inside the window, so the first match is a block starting *after* the split and the slice runs backwards to `''`. The request then carried no scene prose at all — its only prose being the block the model is told to avoid — with `error` empty, so nothing warned: #308 restored by the code fixing #308. `offset` is in the TypedDict so that "empty `read`" is diagnosable at all; it is correct only at `offset == 0`. The unread cap is measured from where the unread prose *begins* rather than from `offset`, since the gap between them could otherwise select a boundary before the block started and read as "nothing follows this image" — an intermediate fix clamped that with `max(blocks[0][1], …)`, which removed the reversed slice by lifting the cap without bound instead, so an ordinary 900+ character first paragraph came back at 2519 and the forbidden block outgrew the read window it must not compete with.

**A `SplitCause` names *why* a position did not resolve**, and it exists because two consumers group and branch on it: `--prompts`' pre-fan-out warning, which renders one clause and one fix per cause, and `spoiler_findings`, which stays silent for the causes `validate_plan` already reports (`CAUSES_WITH_SIBLING_FINDINGS`). Keying either on the `error` prose gave one group per row — every message interpolates a match count, a placement value, a scene id, or an exception — which is `stale_render_reason`'s documented mistake rebuilt by the choice of key. An undrafted scene is `scene_empty` and `unknown`, not `at_scene_end`: that label was both silent and false, since the reader has read nothing there rather than everything, and the paid call went out regardless.

**The renderers take the whole `SceneSplit`, required.** Optional `str = ''` parameters made a forgotten argument indistinguishable from an intentional absence — the failure mode this work exists to fix — and once the header asserted "this is the prose the reader has read", an omission stopped being neutral and became a false claim to the model. A pre-#308-shaped call still type-checked and still ran.

**`split_at_position` normalizes its own input.** Markers and frontmatter both come off inside it. The precondition was previously stated in a docstring, and one of the two production callers did not honour it — so `--embed`, `--prompts`, and `spoiler_findings` computed three different offsets for one row on a legacy frontmatter scene, and the author was shown YAML quoted as "the next sentence the reader reads". A precondition two callers can disagree about should not exist.

**The acceptance line quotes the next sentence** (item 2), and lands in the do-NOT-paste block rather than in Constraints — it is a check on the *render*, and it quotes prose the image model must never see. With `prompt_spoils_unread` it is one of the two fixes that catch a bad render rather than preventing one, which is the point: the failure was invisible to every gate and was found by reading three rows by hand. It renders only for `state == 'normal'` — see the four states below.

**`state_mid_scene_change`** (item 3) is the same root cause from the other side. `state_at` resolves at scene granularity, so the log holds exactly **one value** for an entity across a whole scene — and one value cannot describe both sides of a change. That is LF-13: the Great Lamp's transition at that scene read "the line of light pulled in to just the square, flame shrinking", and the illustration's whole subject was the Lamp dead.

Note what the problem is *not*: `_resolve` compares `<=`, so a transition takes effect **at** its own scene and the resolved value is the transition's declared state, never the previous one. Saying "the log holds the state going in" described a value the resolution cannot return, and invited a maintainer to "fix" a boundary that has a test on it.

**Both edges, symmetrically.** `scene_open` has the same gap with the sign flipped — it precedes the scene's prose entirely, so it resolves to a changed state the reader has not reached. The first cut scoped this to `scene_close` on the reasoning that "every other placement now has a resolvable position inside the scene", which was wrong twice over: a reading position is never fed into state resolution at all, and it excluded the symmetric case. Anchored placements are excluded because an anchor names a phrase, which is at least evidence about where in the scene the image sits.

**Reported, never re-resolved** — guessing "scene_close means after the change" would be wrong for a scene whose change lands in its first paragraph, and this pipeline reports an ambiguous anchor rather than placing art at a guessed offset for the same reason. A `state_override` is the fix and suppresses it, exactly as it suppresses `state_unspecified`.

**`prompt_spoils_unread`** (item 4) covers art already directed, which matters because 17 of 20 rows on the book this was filed about needed re-rendering anyway and every one was directed by a prompt built the old way. Six-word shingles, and **"distinctive" is a set difference, not a frequency heuristic**: `(body ∩ unread) - everything before the split`. Both sides are **all** the prose, not the capped windows the model is sent: subtracting `split['read']` flagged a refrain the reader met early in a long scene and met again after the anchor, and comparing against `split['unread']` made the check silently partial in the other direction — a body quoting paragraph 29 while the image sat at paragraph 5 came back clean, at the same time as `prompt_spoiler_unchecked` was establishing that silence is a verdict. The two windows serve different purposes and only one of them is a claim about coverage. Six is a **correctness threshold, not a tuning knob**: 4 floods a real book with false warnings and 9 stops catching the three rows the issue was filed about, and both left the whole suite green until a boundary test pinned it. Genuinely silent only for a row with no `prompt_file`, and for `establishing` / `at_scene_end` — everything else emits `prompt_spoiler_unchecked`.

All four kinds are **warnings**: the book is publishable either way, two are about art not yet rendered, `prompt_spoiler_unchecked` reports a gap in our own knowledge, and `missing_anchor` matches `anchor_drift` — the same row in the same in-flight condition, where blocking would fail `validate` mid-planning.

**Nothing in `spoiler_findings` logs; every path that cannot check emits a finding.** `illustrations.py` has never logged, and the first cut of this reached for a local `log` import for the unreadable-file path — which put "could not check" and "checked and clean" in the same cell of `working/cleanup-report.csv`, the durable artifact `skills/forge/SKILL.md` scans. `prompt_spoiler_unchecked` is the `canon_staleness_unchecked` precedent applied: an unreadable or **undecodable** file, an unrecoverable or `body_truncated` body, and an unresolved position are all unverified rather than clean. `UnicodeDecodeError` is a `ValueError`, not an `OSError`, so it walked straight out of `validate_plan` — the single finding collector — killing `validate`, `cleanup`, and `--diagnose`; a prompt body hand-edited and saved as latin-1 does it, and hand-editing is documented above as the working fix on a real book. Paired now as `packet._body_for` pairs it, reading the same file. That is the `ill.sha256_of` regression (#298) shape, and a test holds it.

**`missing_anchor` exists because nothing reported it.** `validate_plan`'s anchor checks are gated on the cell being non-empty, so an anchored placement with an empty `anchor` produced no finding anywhere while `insert_marker` refused it on every run — the illustration could never embed, and `spoiler_findings`' claim that "an ambiguous anchor already has its own finding" was false for that whole class.
...
- **A `state_override` written as prose fails three ways at once, and all three are now findings** (#309). Ordinary writing contains semicolons, so a sentence fragments into clauses; the colon-less fragments are dropped; and the one fragment that happens to contain a colon survives with a *sentence* as its entity key, which `state_for_row` hands to the image model as an authoritative state line under that label. `parse_state_override` returns a `StateOverride` (`applied` / `skipped` / `prose_keys`) and **does not log** — two WARNING lines competing with seventeen reference-exclusion warnings were invisible in practice, and they were on the wrong gate, since `--diagnose` and `validate` reported nothing at all. `visual_state.prepass` emits `state_override_unparsed`, `state_override_prose_key`, and `state_override_unmatched_entity` (all warnings; the art renders fine, it is just not the state the author wrote, and the last has a legitimate one-off-entity case). `--prompts` adds a per-row **count** before the fan-out ("1 of 3 clauses applied, 2 lost" — counted while parsing, since `len(applied) + len(skipped)` collapses two clauses naming one entity and under-reported the very losses it exists to count) and **refuses** when the cell did not land as written: nothing parsed, *or* a whole sentence became the entity key. The second disjunct is load-bearing — on the real cell one clause did apply, under a nonsense key, so a nothing-applied test alone let it through. Deliberately **not** keyed on "no key matches a tracked entity": `state_for_row` applies an override for an untracked entity on purpose, so that criterion would break the legitimate one-off entity. The refusal is emitted *after* every other pre-fan-out warning, so it cannot suppress the position report the author needs to fix everything at once. #308 is why this mattered: it made `state_mid_scene_change` recommend `state_override` from a second finding, so the field the pipeline now points at twice was the one that failed quietly when reached for.
...
**Reference images are never copied** into the packet; README's runbook step carries project-relative paths, because a copy is a second thing to invalidate — **and because the author frequently works from a machine other than the one holding the repo**, so a gitignored copy would be the one part of the bundle that does not travel while `manuscript/assets/**` is tracked and already there. The list comes from `--prompts`' own `_references_for`, so the packet inherits the canon-gated exclusions rather than growing a second chain — and `_references_for` takes an optional `notes` out-list so those exclusions are *rendered* under "Read this before you upload", not merely logged. **Those notes aggregate by reason, not by row** (`excluded_stale` is keyed on the `stale_render_reason` string, so two different reasons stay two notes and neither loses the half of the sentence that says what to fix); emitted per row they were the seventeen paragraphs #306 was filed about. The per-file WARNING logs stay per-file — "every exclusion is logged" is a stated invariant, and a log is not the artifact whose signal-to-noise is the feature. That matters more than it looks: with every prior render excluded as pre-canon the list shrinks to the cover, which reads identically to a book where nothing has been rendered yet, and the author then uploads the cover alone and generates the rest with no likeness reference. Cover-only-with-ingested-art is also a README gap. The cap (`_MAX_PRIOR_REFERENCES`) is disclosed too, and is applied *after* the exclusion walk — primarily because you cannot rank candidates you have not collected, and also so a stale render past the fourth prior illustration is not hidden behind a cap that is not why it was dropped.

**Candidates are ranked by anchor-batch slot, then the caller's row order** (#311) — plan order under `--prompts`, *reading* order under `--package`, which passes `rows_in_reading_order`. Phase 1 exists so the churn references four real images, and `--diagnose` will not report "ready to hand over" until every batch slot is ingested from current canon — but selection was a plan-order walk with a cap that never called `packet.anchor_batch`, so once enough post-canon renders existed to fill the cap, **the images the author deliberately rendered and approved were exactly the ones it discarded**. On *The Lantern Folk* the upload list was the establisher plus two arbitrary mid-book images, two of them carrying `prose_changed` warnings, and nothing that bracketed the book's tonal range or locked a later state — three of the four things phase 1 exists to establish. The guarantee phase 1 made was not the guarantee phase 2 consumed.

The rank is a **stable** sort, so a row in no slot keeps the order the caller gave and the change is confined to promoting the batch ahead of them. Promotion order and slot identity come from `packet.slots_by_id` over `packet.BATCH_SLOTS` — one function over the one list of the four slots, extracted because three of the four walks over that tuple wanted the same filled-and-deduplicated intermediate (`render_state`'s argument, applied). `BatchSlot` is a `Literal`, so the `AnchorBatch` subscript narrows and all six `# type: ignore[literal-required]` are gone; a `get_args` totality test catches an addition to one declaration and not the other, and `batch[slot]` replaced a `.get` that was the only one of four consumers to degrade *silently* — it promoted nothing and reverted #311 without a sound. The reference *label* is the slot key (`prior illustration (anchor batch: darkest)`) rather than that tuple's label, whose establisher entry is a sentence written for README's batch table and reads as two clauses fighting inside a one-line label. Keyed by illustration id, not by slot, because a batch whose darkest and brightest resolve to one row — the common case, since nothing populates `register` — must occupy one reference, and is named by the *first* slot it fills.

**Exclusion outranks promotion.** A pre-canon batch member stays excluded rather than being promoted past the reason it was dropped: it was directed by canon that has since been rewritten, and being approved does not change that. The batch is threaded in from the caller (`packet.AnchorBatch`), like `style` and `canon_cutoff` — `_references_for` runs once per row and `anchor_batch` reads the plan, the chapter map, and the transition log. That is a *cost* argument, not a correctness one: `anchor_batch` is a pure read and nothing mutates its inputs mid-run, so two derivations return the same value. What threading buys is one read — `read_transitions` logs a WARNING per malformed row *per read*, and deriving the batch beside a `state_context` that had already read the log made one broken row report itself twice, which is the "N walks read as N broken files" defect the canon-tree walk-count test exists for. So `anchor_batch` takes `plan` / `order` / `transitions` and threads them into `render_order` and `_later_state_exemplar`, and the `batch=None` fallback derives from the caller's own rows, since ranking an in-memory plan against slots read from disk is a precondition two callers could disagree about. Derived, never stored, so a promoted reference cannot disagree with the batch `--diagnose` and README report.

**The cover is additive to the cap, which is the arithmetic #311 surfaced**: cover + a full four-slot batch is five, and the cap was four *total*, so the fourth approved image was dropped by an accident rather than a judgement. `_MAX_PRIOR_REFERENCES` counts prior illustrations only — the cap's own justification ("past that a model starts averaging them") is a claim about like-for-like illustration references, and the cover does a different job (house style, not likeness). Renamed from `_MAX_REFERENCES` so nothing keeps reading it as a total. **A batch image the cap does drop is named as one**, with its slot, in its own note — reported as an anonymous "further ingested illustration" it was the part of #311 that made the bug hard to notice at all. That branch is **unreachable while `_MAX_PRIOR_REFERENCES >= len(packet.BATCH_SLOTS)`** (a test pins the invariant): at most four ids are promoted, each to a rank below four, so no promoted row reaches the dropped tail. It survives on one production route — two plan rows sharing a batch member's `id`, which `validate_plan` reports as `duplicate_id` and `run_package` does not gate — and as the tripwire for lowering the cap or adding a fifth slot, which is `report_batch`'s argument (#290) for keeping a guard whose absence would be silent.

**What fires constantly is the other half, and it went unimplemented in the first cut.** Three claims about the batch were made rather than checked (#311 review):

- **A batch member whose art cannot be used is named with its slot.** A pre-canon one went into `excluded_stale`, which aggregates by `StaleKind` and so cannot say which of those paths were the four the author approved — the disclosure `skills/illustrate/SKILL.md` promises. It also covers the one silent batch-drop that can actually happen: `status=ingested`, a current `ingested_at`, and the file moved or renamed, where `needs_render` never consults `asset_file` so the batch table still reads `Rendered: yes` while the chain lacks it. Deliberately **not** emitted for a member that is merely unrendered — that is valid in-flight state, `--diagnose` already counts those rungs, and a fresh book would get four notes saying nothing actionable.
- **"What is listed is what was approved" is checked against the slots that reached the list.** Unconditional, it was an affirmative falsehood on the ordinary partly-re-rendered batch — 0 of 4 approved images present, under that sentence, in the section whose job is saying the list is thinner than it looks — and it contradicted the dropped-batch note directly above it. The row's own slot is excluded from both sides of the count, or every batch member's prompt would report the batch one short.
- **A guessed slot says so.** Nothing populates `register` on most projects, so darkest and brightest fall back to first and last in reading order; `anchor batch: brightest` for such a row is a claim a prompt file carries neither the batch table nor the `fallback` notes to check. `AnchorBatch.guessed` is set beside each fallback note — one channel a consumer can branch on, one the author reads — and the label appends ", guessed". An *empty* slot is not a guessed one.
...
**The batch has a second consumer since #311:** `_references_for` ranks reference candidates by slot, so `register` no longer only decides what to render first — it decides which images every prompt in the book inherits.
...
**An illustration id must never reach the image model, and `contrast` is the field that broke that** (#305). `contrast_for_row` returns a `packet.RowContrast` with three fields, because the direction has two audiences: **`for_model`** carries no ids ("must not repeat the staging of the illustration immediately before it") and is what `prompt_constraints` renders into the packet's upload file; **`for_author`** keeps them, for the source prompt file's `## Accept only if` and `illustrations.md`, where an id names the file to open; **`withheld`** is the author's own sentences that named an id. The upload file has had no paste boundary since #306, so the derived clause `Follows \`LF-04\` and must not repeat its staging` instructed the model to compare against an image it cannot see, on **19 of 20** rows of a real book. The two phrasings are built from parts rather than by rewriting one into the other — a substitution on the assembled string yields "it Do not repeat…" for the `register` variant, where the clause is mid-sentence. **Author-written `contrast` cannot be rewritten at all**: "colder and markedly darker than \`LF-05\`" has no meaning without `LF-05`, so those sentences are withheld from the upload entirely and surfaced in `illustrations.md` as **Check against its neighbours** — a check on the render rather than a direction for it. `build_art_direction_request` receives `for_model` too, because the model *writing* the prompt would otherwise echo an id into the body, and the body is uploaded verbatim. Ids are matched against `RowContext.plan_ids` (which **includes `superseded` rows**, so it is not `set(predecessors)`) plus any backticked token shaped like one; a generic pattern alone is far too eager, since a plan id may be as plain as `a10`. A test asserts no upload file names any illustration but its own **for the contrast path**; the other routes — a pre-#305 prompt body, and `composition` / `beat` / `state_override` / `absent` typed by hand — are *reported* as gaps by `packet._foreign_ids` rather than rewritten, since no edit preserves the meaning of "deliberately unlike `LF-07`'s hard light". Matching goes through `packet._id_pattern`, shared by both consumers: `\b` is the wrong boundary for these ids and missed `LF-07s`, `_LF-07_` and a trailing-hyphen id, all of which leaked. `packet._backticked_ids` covers a contrast naming a **cut** row, which `plan_ids` cannot know about — restricted to plan-id *shape* and excluding this domain's own vocabulary (`half_page`, `scene_open`, `darkest`, plan columns), because the first version of that fallback matched any backticked token and withheld `Keep the \`half_page\` gutter clear.` *while telling the author it named another illustration*. A wrong explanation for a drop is worse than the drop.
