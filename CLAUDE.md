...
| `storyforge score` | `cmd_score.py` | Craft scoring (25 principles + fidelity scoring against briefs). `--principles P1,P2` for targeted scoring; deterministic principles (`prose_repetition`, `avoid_passive`, `avoid_adverbs`, `no_weather_dreams`, `sentence_as_thought`, `economy_clarity`) skip the LLM pipeline entirely. **Elaboration entry points:** `--level N` (0-6) runs floor checks; `--all-levels` runs every level; `--compare a b [c]` produces a multi-candidate report at the prose tier (no winner declared); `--compare ... --semantic` populates LLM ceiling axes (specificity, irony, hook word, etc.); `--drift` produces a read-only deterministic drift report; `--boundary N->M` runs the LLM faithfulness diff at one level boundary (with optional `--scope`); `--all-boundaries` runs every boundary; `--bible-consistency` checks each drafted scene against the project bibles (cached, ~$20-25/run); `--story-power` runs the 8-axis story-power scorecard (specificity, emotional resonance, character identification, stakes, archetypal resonance, thematic depth, surprise, moral weight) on the pitch artifacts (logline/synopsis/theme/spine/architecture), coaching-aware, with delta tracking across runs (see `references/story-power-rubric.md`). All scoring respects `working/scoring-overrides.csv` — accepted findings surface tagged but don't count toward failure totals. |
