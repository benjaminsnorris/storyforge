...
| `export.py` | The self-contained per-illustration export: `resolve` (units, references-as-copy-instructions, and the same `gaps` record), the body recovered from `prompts/{id}.md` or derived from the plan row, `manifest_for`, `copy_references`, `prune_units`, `export_stale`. No rendering, no LLM |
...
**The export re-derives the Constraints block; it inherits only the body.** `prompts_illustrate.parse_prompt_file` recovers the model-authored prose and stops at `### Constraints`, and `prompt_constraints` regenerates the rest from the plan. That split is load-bearing: there is no `prompt_stale`, so a prompt file written before a matrix edit still carries the old state in its own Constraints block — the export exports the state in force *now*, and a block that contained both would contradict itself, which is #297 inside the fix. The parse is structural (the paste sentinel and the Constraints heading, both module constants shared with the writer) rather than marker-delimited, because HTML-comment markers around the body would sit *inside* the region the file tells the author to paste.

**Reference copies are the point, and the recorded sha256 is what makes them safe.** The packet's objection to copying — a second thing to invalidate — is answered rather than dismissed: `manifest.json` records each copy's digest and `export_stale` hashes the source again, so a replaced render is reported. That is the one way this bundle goes stale with nothing under `reference/` touched, which is why it is its own finding kind rather than a second `packet_stale` whose detail would name the wrong command. Symlinks are resolved on copy (`copyfile`, not `copy2` — an inherited mtime would make the copy look older than the export that wrote it), and the resolved target is recorded, because a zip preserving a dangling link is worse than a copy. A source that resolved and cannot be *read* is skipped with a note rather than raising: `_references_for` resolves with `isfile`, so a dangling link and an absent file never reach the copy, and a manifest promising an upload the directory does not hold is the one failure the export exists to make impossible.
...
**A whole-plan export prunes; a subset export never does.** `prune_units` deletes directories for rows that have left the plan — but only directories holding a `manifest.json` this command wrote, so an author's own notes survive. A `--ids` run has no business deleting units it was not asked about, so `README.md` names them as untouched and possibly built from sources that have since moved. Saying "1 illustration" over a directory holding twenty is the coverage overclaim the gap section exists to prevent.
