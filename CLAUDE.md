...
**Reference images are never copied** into the packet; `reference-images.md` carries project-relative paths, because a copy is a second thing to invalidate. The list comes from `--prompts`' own `_references_for`, so the packet inherits the canon-gated exclusions rather than growing a second chain.
