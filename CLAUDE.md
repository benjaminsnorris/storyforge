...
**The governing rule inverted, because the author uploads the file rather than pasting a region out of it.** It was *"mark what must not be pasted"*; it is now **everything in an image prompt is for the model, or it is not in the file**. The export's per-unit file was 13,940 bytes of which **9,500 were seventeen near-identical paragraphs about canon staleness**, all above a paste boundary an upload ignores. Four of the five sections that left were byte-identical across all twenty files, which is the tell that they were never per-illustration facts: the reference-chain disclosures and the provenance sentence went to `README.md` once, the upload list went to README's runbook step, the "Read this first" blockers and the assigned staging went to `illustrations.md`.
