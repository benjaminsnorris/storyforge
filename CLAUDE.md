...
**Severity `error`, but nothing gates on it.** Same class as `canon_id_mismatch` in that both break prompt assembly where the anchor is consumed — the failure modes differ, and truncation's is worse: a `canon_id` mismatch fails the lookup and surfaces as an unanchored row that `_warn_unanchored_rows` announces, whereas a truncation hands every consumer a shorter string they all accept, and once art exists the only repair is a re-render. Reachability is narrow and worth knowing: `cleanup` is the **only** command that runs canon validation (`cmd_validate` gates on `schema.validate_illustration_plan`, not `canon.validate_canon_directory`; `status` and `forge` never read canon findings), and `cmd_cleanup.main` returns `None` on every path. So this prints and writes a report row; it blocks nothing. Closing that is a follow-up.
