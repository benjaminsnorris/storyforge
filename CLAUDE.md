...
| `packet.py` | What goes in the handoff packet and proof the copies are exact: `resolve` (entries, anchors, references, and the `gaps` coverage record), `rows_in_reading_order`, `state_grid`, `anchor_batch`, `anchor_block`, `anchor_copy_drift`, `packet_stale`. No rendering, no LLM |
...
**A `##` heading inside an Embeddable block is an error, not a silence.** `embeddable_block_text` reads to the next `##`, so an author who sub-heads the anchor itself (`## Wardrobe`) loses every word below that line — and the anchor is the string every prompt embeds verbatim, so the images then drift on whatever the dropped tail described. **The byte-identity checks cannot catch this**: `tests/test_packet.py` and `tests/test_illustrate_package.py` both compare against `anchor_texts`, the truncating function, so every check agrees with every other about a value already wrong. The extractor is deliberately *not* widened — reading past the `##` would swallow whichever section actually followed and feed it to an image model as description — so the truncation stays and `canon_truncated_embeddable_block` reports it (**error**, same class as `canon_id_mismatch`: downstream consumers get wrong data with no other sign, and once art exists the only repair is a re-render). `canon.embeddable_block_truncations` finds the offenders; its window closes at the first `REQUIRED_SECTIONS` heading after the block, since past that point a non-schema heading is somebody's extra section rather than lost anchor text. Its terminator regex is `_EMBEDDABLE_BLOCK_RE`'s lookahead character-for-character and is matched against the **whole body**, not per line: that `\s` is usually the line's own newline, so the plausible per-line spelling `^##(\s|$)` diverges on a bare `##` as the final line with no trailing newline — which does not truncate and must not be reported. `_SECTION_RE` is the wrong enumerator for the same reason (it needs `##` plus a name, so a bare `##` line would slip past it while still truncating). `ParsedCanonFile.body_line_offset` exists so the finding can name the file line the author will actually look at.
