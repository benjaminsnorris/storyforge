...
def export_outline_md(project_dir: str, output_path: str | None = None) -> str:
    """Render the expanding outline into a single read-only markdown file.

    Three numbered sections: Spine, Architecture, Scenes. The `summary`
    column from each CSV becomes the numbered-list body. When briefs
    exist for a scene, a "Brief:" sub-bullet shows what the brief
    promises (goal → outcome), so the outline keeps expanding past
    scenes into the drafting layer.

    Renders even when one or two source CSVs are absent. Returns '' only
    when none exist. Output defaults to reference/outline.md.
    """
    spine_csv = os.path.join(project_dir, 'reference', 'spine.csv')
    arch_csv = os.path.join(project_dir, 'reference', 'architecture.csv')
    ...
    lines.append('<!-- Derived from the `summary` columns of spine.csv, '
                 'architecture.csv, scenes.csv, plus brief goal/outcome '
                 'from scene-briefs.csv. Edit the CSVs; this file is '
                 'regenerated by `storyforge sync`. -->')
    lines.append('')

    briefs_by_id = _briefs_by_id(briefs_csv)
    ...
    for label, csv_path in (
        ...
        if not os.path.isfile(csv_path):
            ...
            lines.append('')
            continue
        rows = _outline_rows_with_ids(csv_path)
        if not rows:
            lines.append('_(no rows yet)_')
            lines.append('')
            continue
        for seq, summary, row_id in rows:
            marker = str(seq) if seq is not None else '?'
            lines.append(f'{marker}. {summary or "_(missing)_"}')
            # Only scenes get a brief sub-line, and only when the brief
            # carries enough content to form a useful promise.
            if label == 'Scenes' and row_id in briefs_by_id:
                brief_line = _format_brief_promise(briefs_by_id[row_id])
                if brief_line:
                    lines.append(f'   - Brief: {brief_line}')
        lines.append('')
...
def _format_brief_promise(brief: dict) -> str:
    """Return a one-line summary of what this brief promises, or '' if
    too sparse to be useful.

    Builds from goal + outcome (the brief's pursuit→result axis). For
    sequel-shaped scenes (no goal but a decision present), falls back to
    decision."""
    goal = brief.get('goal', '').strip()
    outcome = brief.get('outcome', '').strip()
    decision = brief.get('decision', '').strip()
    if goal and outcome:
        return f'pursue "{goal}"; result "{outcome}"'
    if goal:
        return f'pursue "{goal}"'
    if outcome:
