...
COMMAND="${1:?Usage: ./storyforge <write|evaluate|revise|assemble|review|test> [options]}"
shift
...
case "$COMMAND" in
  ...
  *)
    ...
    if [[ ! -x "$SCRIPT" ]]; then
      echo "Error: Unknown command '$COMMAND'. Available: write, evaluate, revise, assemble, review, test" >&2
      exit 1
    fi
    exec "$SCRIPT" "$@"
    ;;
